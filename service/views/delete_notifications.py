...
def index():
    ...
    try:
        ...
        return render_template('delete_notifications/index.html', publisher_id=publisher_id,
                        publisher_ids=publisher_ids, since=since, upto=default_upto,
    ...
    if user and password:
        ...
        render_template('delete_notifications/index.html', publisher_id=publisher_id,
                        publisher_ids=publisher_ids, since=since, upto=default_upto,
                        status_values=status_values)
    headers = {
    ...
    data = {
        "conf": {"upto": upto, "from": since, "status_values": status_values, "publisher_id": publisher_id},
        "note": f"User request to delete notifications between {since} and {upto}"
    }
    command = "dagRuns"
    api_url = f"{airflow_rest_url}{deletion_dag}/{command}"
    r = requests.post(api_url, headers=headers, data=json.dumps(data))
    if r.status_code >= 200 and r.status_code < 300:
        flash(f"Successfully triggered Airflow DAG to delete notifications between {since} and {upto}.")
    else:
        ...
        return render_template('delete_notifications/deletion_sent.html', publisher_id=publisher_id,
                              publisher_ids=publisher_ids, since=since, upto=upto,
                              status_values=status_values)
    ...
    return render_template('delete_notifications/deletion_sent.html', publisher_id=publisher_id,
                           publisher_ids=publisher_ids, since=since, upto=upto,
                           status_values=status_values, airflow_url=airflow_display_url)
