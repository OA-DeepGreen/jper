...
def get_all_notifications(index, since, upto, publisher_id, log_url=None):
    ...
    for hit in records["hits"]["hits"]:
        ...
        c = b.pull_record_for_notification(notification_id)
        if not c: # No routing history found - create a new record
            app.logger.info(f"No routing history for notification {notification_id} - creating a routing history record now")
            create_routing_history_record(index, note_id, log_url=log_url)
...
def clean_old_data():
    ...
    def list_notes_of_publisher(publisher=None):
        ...
        if b.retain_routed <= 0:
            ...
            index = "jper-routed*"
            notes_routed = get_all_notifications(index, since, upto, publisher_id=publisher, log_url=log_url)
        ...
        if b.retain_failed <= 0:
            ...
            index = "jper-failed*"
            notes_failed = get_all_notifications(index, since, upto, publisher_id=publisher, log_url=log_url)
                ...
                if num_records == 0:
                    ...
                    for hit in records.get('hits', {}).get('hits', []):
                        for note_state in hit.get("notification_states", []):
                            ...
                            if notification_id:
                                notes_errored.append(notification_id)
        ...
        print(f"notes_routed: {notes_routed}, notes_failed: {notes_failed}, notes_errored: {notes_errored}")
        return (publisher, notes_routed, notes_failed, notes_errored)
    ...
    def notes_list_all_publishers(files, session=None):
        ...
        if files: # Protect if there are no files to transfer
            for file in files:
                publisher_id = file[0]
                for routed in file[1]:
                    notes_to_delete.append((publisher_id, routed, "success"))
                for failed in file[2]:
                    notes_to_delete.append((publisher_id, failed, "failed"))
                for errored in file[3]:
                    notes_to_delete.append((publisher_id, errored, "errored"))
        else:
            app.logger.warn("Empty run")
            dag_run = session.merge(context['dag_run'])
            dag_run.note = "Empty run"
    ...
    def delete_one_notification(note_tuple):
        publisher_id, note_tuple, status = note_tuple
        note_id = note_tuple[0]
        index = note_tuple[1]
        app.logger.info(f"Deleting notification {note_id} for publisher {publisher_id} with status ({status})")
        context = get_current_context()
        log_url = get_log_url(context)
        a = RoutingDeletion(publisher_id=publisher_id, note_id=note_id, index=index, log_url=log_url)
        a.airflow_log_location = log_url
